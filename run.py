...
def set_model(args, dataset):
    ...
    if config in ['bow', 'lstm', 'pipe_transformer']:
        if config == 'bow':
            node_model, tree_model = ('bow', None)
        elif config == 'lstm':
            node_model, tree_model = ('lstm', None)
        elif config == 'pipe_transformer':
            node_model, tree_model = ('transformer', None)
        ...
        args.log_name = 'bow' if config == 'bow' else 'bow_lstm'
...
def main(args: Namespace) -> None:
    ...
    if args.skip_setup:
        ...
        args.trees = build_trees_and_files(directory=args.setup_location, pandas=args.pandas, max_trees=args.num_trees)
        # if args.build_trees:
    ...
    logger = TensorBoardLogger('res_and_ckpts/tb_logs', name=args.log_name + '_' + args.run_name)
    # logger = pl_loggers.CometLogger("comet_logs/")
    if args.num_gpus > 0:
        model = model.cuda()
    trainer = Trainer(
        gpus=args.num_gpus,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pandas', type=bool, default=True)
